...
- 桌面正文居中且不被索引展开挤压；正文宽度支持窄、标准、宽三档并跨浏览器同步。
- 对话索引支持悬停预览与点击锁定，章节目录只在空间充足时常驻。
- 手机端采用全宽长文阅读，而不是狭窄的左右聊天气泡。
- 阅读区支持方向键、空格、PageUp/PageDown、Home/End 和 J/K 导航；`Ctrl/Cmd+F` 打开当前会话搜索。
...
- Conversation 和 Project 默认按最近阅读排序，支持更新时间、创建/导入时间、标题、数量及拖动自定义顺序；排序偏好跨浏览器同步。
...
- 搜索 conversation、message、heading 和 code，可限制 Project、角色、归档状态、日期或当前会话；heading/code 结果精确定位到 block，merged/source 中内容相同的消息默认合并显示出现次数。
