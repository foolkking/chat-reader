...
- Production Chrome formula-scroll recheck was not completed in this deployment session because the connected Chrome bridge became unavailable while claiming the existing QA tab. This remains `NOT_PRODUCTION_VERIFIED`; browser evidence must not be inferred from the API/container health checks. The browser-independent focused layout tests and production build passed before deployment.
