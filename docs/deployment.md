...
生产 healthcheck 使用低频配置，避免小内存主机频繁创建 Docker exec：PostgreSQL 每 30 秒，API/Web 每 60 秒。容器日志启用 10 MiB、3 文件轮转。Web 远程构建的 Node heap 上限为 640 MiB；1 GiB 以下服务器应顺序构建，并在构建 Web 前停止现有 API、worker 和 Web 容器。
