...
当前生产路径使用 `docker-compose.production.yml`：PostgreSQL、一次性 migration、FastAPI 和 Next.js 四个服务。只有 Web 端口对宿主机暴露。
...
docker compose --env-file .env.production -f docker-compose.production.yml logs -f migrate api web
```
...
Nginx 的 `client_max_body_size` 应大于 `MAX_IMPORT_FILE_SIZE_MB`，长导入请求需要足够的 proxy timeout。
...
4. 确认 migrate completed、API/Web healthy。
...
```bash
docker compose --env-file .env.production -f docker-compose.production.yml ps
docker compose --env-file .env.production -f docker-compose.production.yml logs --tail=200 api web postgres
docker compose --env-file .env.production -f docker-compose.production.yml restart api web
```
