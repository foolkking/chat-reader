...
| A11Y-001/A11Y-002 dialog focus/close | Dialogs had per-component lifecycle, backdrop buttons and no shared focus restoration. | Added `useDialogFocus` and migrated import, interaction, new conversation, insert and file details dialogs; backdrop is non-focusable and exactly one visible close control remains. | PASS: compile/lint; dedicated full online keyboard run remains gated. | NOT_PRODUCTION_VERIFIED after this release. |
| UI-001/COPY-001 | DisabledScanner note used warning styling; project create trigger exposed English metadata and did not autofocus. | Neutral Info/secondary `未扫描`; Chinese `新建项目` accessible label/title and autofocus input. | PASS: compile/lint. | NOT_PRODUCTION_VERIFIED after this release. |
...
- 360px, 125/150/200% zoom, two-tab race and forced-offline negative paths: NOT_PRODUCTION_VERIFIED.
