...
## Post-Closure Attachment Workspace And Markdown Cursor Verification - 2026-08-11

- A later UX correction superseded the fixed-left Files placement: desktop `当前对话文件` now uses the annotation-style right-side floating workspace, with the full header as a `grab` drag handle and an accent attachment icon. Production Chrome verified the reset geometry and computed styling; mobile behavior did not change.
- The source jump was reproduced without saving business data. Root cause combined unstable CodeMirror configuration props, Reader's global keyboard intent accepting editor key events, and a controlled-value draft echo. The correction stabilizes CodeMirror configuration, excludes editable targets from Reader keyboard intent and separates the external editor document from live draft state.
- Production Chrome type/delete evidence: cursor `21860 -> 21861 -> 21860`, scrollTop constant at `41091`, same source message throughout. This is higher-level evidence than the earlier static source-editor checks.
- Deployment evidence: commit `1cdadc4f90115d7b46ce55d07a2b4f23c90471d4`, Actions run `31470442426`, archive SHA-256 `429fb5384dc1dbf57eec68aecad4632c01bd71a58fca6ea9f276468c6d8630fb`, validated backup `/opt/chat-reader/backups/file-workspace-cursor-20260811T075200Z-1cdadc4`. Services remained healthy after retaining current plus one rollback image set and deleting 48 exact old tags.
