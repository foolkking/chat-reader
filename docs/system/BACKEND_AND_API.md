...
The source editor uses `cr-upload://<upload-item-uuid>` only as an unsaved draft marker. `PATCH /api/messages/{id}` requires every such marker to name an item in `upload_item_ids`, rejects non-UUID draft tokens and rejects any marker left after replacement, with a source line number in the 422 detail. The route then rewrites each item to `cr-asset://<attachment-uuid>` inside the same transaction used by message version, render block and occurrence persistence. Attachment references written as `cr-asset://` are checked for current conversation ownership; invalid or cross-conversation references likewise report the source line.
