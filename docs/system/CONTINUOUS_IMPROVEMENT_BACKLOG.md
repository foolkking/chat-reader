...
| CI-013 | CI | Add explicit timeout and artifact collection for every long browser gate. | Strong: first deployment attempt timed out in optional SW readiness. | H | M | Strong | M | L | Candidate |
| CI-014 | CI | Publish deployable artifact names with commit SHA and run attempt instead of a constant name. | Observed: workflow artifact is always `chat-reader-images`. | M | M | Observed | S | L | Candidate |
...
| DOC-004 | Documentation | Add a current troubleshooting entry for IndexedDB bulkPut AbortError and recovery steps. | Observed: user encountered this exact error. | M | H | Observed | S | L | Candidate |
...
| Completed in this backlog | 7 |
| Blocked | 0 |
| Remaining candidates | 93 |
