...
- 2026-08-09 Attachment Viewer candidate 增加 PDF.js/browser Viewer chunks、bounded derivatives、text search 和 worker batch ZIP。它仍须由 CI/外部 Linux 主机构建镜像；King 只允许备份、拉取对应提交、校验镜像、运行 migration preflight 和 `docker compose --no-build` 更新。不得在 King 运行 `next build`、启动 ClamAV、覆盖 `.env.production` 或删除 volume。该 candidate 当前为 `NOT_PRODUCTION_VERIFIED`。
