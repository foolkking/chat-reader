...
Each occurrence records detector version, match mode, confidence, evidence codes and optional similarity in addition to its location. Exact/normalized structural matches and exact user rules are high confidence. Broken wrappers and fuzzy syntax tokens are medium confidence; approximate user literals and partial selections are low confidence. Only high-confidence candidates may be preselected. Medium and low confidence candidates remain `KEEP` unless the owner explicitly selects them.
