...
Share bootstrap 只返回分享和会话元数据。正文、对话索引、当前消息 TOC 与 heavy blocks 分别通过 token 约束的分页接口读取；每个接口都执行 scope 校验，避免分享链接越权读取未选择消息。主阅读器的阅读位置按服务端 `subject_key` 保存，当前单用户身份为 `local:default`；Share 访客位置仅保存在浏览器本地。
