...
## 2026-09-01 Conversation list and settings resilience

- The All Conversations and Project conversation sortable shells no longer
  advertise their empty metadata area as an interactive row. Idle wrappers use
  a neutral cursor/state; the existing content row remains responsible for
  selection, focus and drag visuals.
- Conversation navigation indexes now prefer the server projection but fall
  back to the canonical reader message window while that auxiliary projection
  is empty or catching up after a refresh/import/merge. This prevents a false
  “no dialogue messages” state without creating new persistence or APIs.
- Settings no longer repeats the removed Appearance & reading heading. The
  existing upward-expanded settings region labels the secondary controls as
  “More reading settings” and orders categories as Account & security, Data
  archive, Skill management, Import formats, and Noise rule library.
- Verification: Web typecheck, lint, production build, TOC refresh contract
  test and `git diff --check` passed. Authenticated browser acceptance and
  deployment were not performed in this cycle.
- The current backend contract now includes a compact data-boundary map for
  Owner Reader, public Share, Offline Reader, Skills, and `.cr` restore. It
  records permitted projections and forbidden crossings without storing user
  conversation content; DOC-009 is completed and the backlog has 32 remaining
  candidates.
- Release documentation now requires an independent owner-authenticated browser
  status (`PASS`, `NOT_VERIFIED`, or `BLOCKED`) for every release, keeping
  production-equivalent and anonymous health evidence separate. DOC-008 is
  completed; this cycle still has no approved owner session or deployment.
- Reader target navigation reuses the center turn returned by target-context
  resolution and loads only neighboring turns; in-flight maps prevent duplicate
  concurrent target/turn requests. The focused Reader location contract covers
  this path, so LOC-006 is closed as already implemented. Backlog remaining
  candidates: 30.
- Artifact cleanup policy is already explicit in `docs/system/RETENTION_CONTRACT.md`:
  automatic deletion remains disabled, reports are dry-run only, and any future
  apply requires category/token confirmation plus a fresh race-safety recheck.
  DOC-010 is closed as already implemented; remaining candidates: 29.

- `docs/system/RELEASE_EVIDENCE_INDEX.md` now maps required quality, migration,
  browser and production gates to source-bound evidence locations and keeps
  `AUTOMATED_TESTED`, `BROWSER_VERIFIED`, `PRODUCTION_VERIFIED` and
  `NOT_VERIFIED` distinct. TEST-010 is completed; backlog remaining candidates:
  28. No deployment or commit was performed in this cycle.

- Deployment verification labels were already explicit in
  `docs/system/DEPLOYMENT_AND_ENVIRONMENT.md`: production-equivalent owner
  runs, deployed production evidence and `NOT_VERIFIED`/`BLOCKED` are kept
  separate. DEP-009 is therefore closed as already covered; backlog remaining
  candidates: 27.

- `docs/index.md` current verification date is aligned to the latest recorded
  2026-08-31 deployment evidence. DOC-001 is completed; this cycle remains
  local-only with no new deployment, and backlog remaining candidates: 26.

- Web feature test ownership is now checked by
  `apps/web/e2e/feature-test-inventory.spec.ts`; all 13 feature directories are
  mapped to a named browser/API contract and the Playwright inventory check
  passed 1/1. TEST-009 is completed; backlog remaining candidates: 25.

- External browser acceptance now has a repeatable preflight:
  `corepack pnpm run verify:chromium` validates the operator-provided absolute
  Chromium executable and version without persisting its local path. DEP-008 is
  completed; CI continues using bundled Chromium and backlog remaining
  candidates: 24.

- Browser release evidence already has per-gate JSON plus a stable aggregate
  summary with source-SHA metadata; the workflow uploads both. CI-020 is closed
  as already implemented; backlog remaining candidates: 23.

- Deployment now includes a read-only PostgreSQL continuity check:
  `deploy/verify_postgres_unchanged.sh` compares the recorded container
  `StartedAt` and optional ID after an API/Web rollout. DEP-003 is completed;
  the script was not run against production in this local-only cycle and
  backlog remaining candidates: 22.

- Artifact cleanup already has the required manual, race-safe apply contract:
  category/token confirmation, fresh per-object revalidation and accurate
  partial results; automatic cleanup remains disabled. CI-008 is closed as
  already implemented; backlog remaining candidates: 21.

- Rollback verification now has a read-only composite smoke command,
  `deploy/verify_rollback_smoke.sh`, for health, anonymous private-route 401
  and migration-head checks. DEP-007 is completed; it was not run against a
  production rollback in this local-only cycle. Backlog remaining candidates:
  20.

- Release quality now records standard and PWA-negative Web build elapsed time
  and peak RSS in source-bound artifacts, enabling later cache comparisons
  without changing deployable outputs. PERF-008 is completed; backlog remaining
  candidates: 19.

- Verification for this documentation/verification cycle: Web typecheck and
  lint passed; the full API suite passed `443` tests with `6` skips; the focused
  feature-test inventory and Chromium preflight also passed. No commit or
...
- The durable candidate queue is [Continuous Improvement Backlog](docs/system/CONTINUOUS_IMPROVEMENT_BACKLOG.md): 134 evidence-labeled items, 99 completed/implemented/closed, 35 remaining candidates, 0 blocked.
- Project and Conversation drag affordances were revalidated as one custom dnd-kit contract: native link drag is disabled, full rows activate drag, non-link overlays retain source dimensions, and cancellation produces no reorder mutation. The combined production-equivalent Chromium check passed 1/1; no additional runtime change was required.
- The owner-shell browser contract also records document navigation: across Home → Conversation → Project → Recent → Search, only the initial Home load issues a `document` request while subsequent App Router transitions replace embedded正文 content and retain the same Sidebar DOM. The focused Chromium check passed 1/1.
- Persistent owner-shell navigation now has a direct DOM-identity browser contract. The root `WorkspaceShellBoundary` keeps one Sidebar mounted while Home, Conversation, Project, Recent, and Search replace only their embedded content; a non-serializable probe survived every client navigation in the focused mock Chromium run (1/1). Login, Share, Offline, and Library remain intentionally separate boundaries.
- Imported-set access already uses the bounded result data instead of a persistent ImportBatch product: synchronous completion exposes per-Conversation entries (up to eight), global Tasks retains compact Import result links, and larger sets fall back to the ordinary Library. The default mock Chromium batch contract now asserts a direct second-Conversation entry and passed 1/1; no runtime change was required.
- Mixed Adaptive Import sessions already preserve completed format work across sibling recovery: excluding an invalid input keeps the valid source and reaches READY, while replacing an invalid sibling reuses the verified Family format after reanalysis instead of asking for Mapping again. The focused exclude/replace contracts passed 2/2; no runtime change was required.
- Batch Import terminal behavior now has a default mock Chromium contract independent of the conditional full-stack fixture. A two-item commit remains on the opener, reports 2 Conversations/4 messages plus its warning, exposes View imported conversations, Open first, and Close, and navigates only after explicit Open first. The browser contract passed 1/1; delayed/partial work continues to use the global Tasks result contract.
- Adaptive Import Overview now consumes the required server `handling_class` directly for all grouping and CTAs; the former `resolution_status` fallback inference was removed, leaving lifecycle/profile status explanatory rather than authoritative. API Adaptive Import tests passed 32/32 and Web lint/typecheck passed. The conditional full-stack Import browser flow was discovered but skipped because its isolated `E2E_IMPORT_FLOW=1` environment was unavailable; browser acceptance is NOT VERIFIED for this local cycle.
- Adaptive Import now enforces `NOT_MAPPABLE` as a server capability boundary for Mapping preview/save and saved-Profile selection. Direct API attempts return `FAMILY_NOT_MAPPABLE`, preserve the classification, and leave `mapping_draft` empty rather than producing a fake canonical preview or Learned Profile. Focused tests passed 2/2 and the full Adaptive Import file passed 32/32. This is local only and not committed or deployed.
- Same-block attachment occurrences are protected by an explicit owner/Share Reader API regression. The current projections already use ordered occurrence lists and `attachmentOccurrences[]`; the focused test places two identities on one current block and confirms both survive in owner and public Share output without exposing storage paths. The test passed 1/1; no runtime change was required.
- Attachment-reference repair guidance now defines the privacy and manual-review boundary: durable evidence contains aggregate codes/counts rather than real IDs or body text; missing, ambiguous, filename/order-only, or conflicting mappings must remain manual/NOT VERIFIED. Any evidence-backed repair requires a current database/asset backup, a new MessageVersion, a post-repair read-only audit, and Reader/Source/Files/Viewer/Search/TOC checks. No user data was inspected or repaired in this documentation-only cycle.
- Offline package compatibility now has focused production-equivalent Chromium coverage: a legacy v1 package imports and renders through the Offline Reader, a v2 zero-message/count mismatch is rejected without replacing the prior readable copy, and an injected IndexedDB transaction abort preserves the active package/cache before an idempotent retry. The focused matrix passed 3/3; existing Library PWA tests continue to own cold-start coverage. This is local only and not committed or deployed.
- TEST-007 was revalidated and closed as already covered by the current merge regression suite. `test_merge_history_and_cancellation.py` plus `test_split_merge_api.py` passed 17/17 across attachment UUID rewrite/occurrence integrity, RenderBlock/hash projections, source immutability, message/detail/Source data, Files, Viewer, Search, TOC, Offline package output, and fail-closed rollback. Authenticated browser and production verification remain NOT VERIFIED; no duplicate test was added.
- Reader location failure now has an independent mock Chromium contract: an out-of-range block target and a 404 target message both preserve the safe Reader article, expose retry/message recovery actions where appropriate, and leave no stale locate pulse after failure. The contract passed 2/2; authenticated production stale-version migration remains NOT VERIFIED. This is local only and not committed or deployed.
- Drag overlays now preserve the source row's real dimensions even when dnd-kit's drag-start `initial` rect is unavailable. The first production-equivalent Chromium run reproduced a 14px project-overlay height mismatch; after adding translated/real-DOM fallbacks across Sidebar, All Conversations, and Project Conversations, project and conversation overlays stayed within 2px and Escape cancellation emitted no reorder mutation. Web lint/typecheck/build and the focused browser contract passed; this is local only and not committed or deployed.
- Delayed row previews now cancel even during their 700ms waiting period when scrolling, resizing, starting DnD, or pressing Escape. Production-equivalent Chromium verifies the timing threshold, full containment in a 480x280 viewport, Escape and pending-scroll dismissal, touch suppression, and both All Conversations and Project Conversations call sites. Web lint/typecheck/build and `git diff --check` passed; this is local only and not committed or deployed.
- Stale attachment-reference navigation now remains visible instead of being overwritten by `idle` after fallback. Attachment targets carry explicit source identity; stale/failed states preserve Reader content and provide exact retry, message-level fallback, and a Files-reference refetch that does not pretend to repair canonical data. Attachment policy contracts passed 7/7, Web lint/typecheck/build passed, and `git diff --check` passed. A real owner-data stale-reference interaction is `NOT VERIFIED`; this is local only, not committed or deployed.
- Offline package bulk writes are now directly regression-protected at the original 15,448-record failure scale. The production chunk helper emitted 155 requests, never exceeded 100 records, retained all records, and finished with a 48-record batch; the existing synthetic AbortError flow also preserved the previous package and Cache Storage asset before a successful retry. The isolated negative-PWA build, both Chromium contracts, Web lint and typecheck passed. True cross-retry checkpoint journaling is still the separate `OFF-003` candidate; no journal or partial-package state was added here.
- Historical backup retention now has a bounded, read-only inventory command: `python deploy/backup_retention_report.py`. It keeps explicit/latest/recent categories distinct from review-only older complete backups, holds incomplete or unknown entries, defaults to aggregate output, refuses truncated scans, never follows top-level symlinks and never deletes. Four focused tests, Python compilation and `git diff --check` passed. It has not been run against local or production backup directories and is not committed or deployed.
- All Conversations and Project Conversation rows now keep their whole-row hover affordance when custom sorting is disabled. Hover ownership is independent from dnd-kit activation, so recent-read sorting retains a pointer cursor, full opacity, subtle surface shift and normal link navigation instead of looking disabled. Web lint, typecheck and production build passed; the focused production-equivalent Chromium test passed both list routes. This is implemented locally, not committed, deployed, or authenticated-production verified.
- Routine Export/Offline cleanup debt reporting is now bounded and aggregate-only. `python -m scripts.artifact_cleanup_dry_run` scans at most 100,000 files by default, reports scan completeness plus eligible category counts/bytes, exits 2 rather than presenting a truncated scan as complete, and only fails on observed debt when `--fail-on-debt` is explicit. It emits no path or candidate token and does not change the existing separately approved, identity-bearing apply flow. Cleanup/lifecycle coverage passed 17 tests with 1 platform-specific symlink skip; no cleanup was executed against local or production artifacts.
- Local attachment storage now has a standalone, read-only pre-deploy reconciliation command: `python -m scripts.verify_attachment_storage`. It checks active Attachment-to-AssetObject ownership, local storage-key containment, available-file existence and size, optional SHA-256, misplaced available objects, orphan object files and bounded-scan completeness. Technical staging/cache directories remain owned by lifecycle cleanup; the command never repairs, moves or deletes data and defaults to aggregate-only output. Four focused tests and the complete API suite (430 passed, 6 skipped) passed; Alembic remains at the single `20260829_0029` head. The command has not been run against the unidentified local services or production assets.
- Adaptive Import no longer collapses every non-mappable Family to the same generic explanation. The existing Overview row keeps an input-type identity and now renders the server-owned reason title plus its bounded, content-safe recovery detail, so invalid JSON, ordinary documents and instruction packages explain why Mapping cannot solve them without exposing source text or internal error codes. Adaptive Import API passed 32/32, Web lint, typecheck and production build passed; the focused Import browser flow is `NOT VERIFIED` because the available API/PostgreSQL processes are not an identified disposable test environment.
- One synthetic attachment merge now provides cross-Surface regression evidence for the rewritten target identity and occurrence through Reader message-window, message detail/Source data, Files, authorized Viewer bytes, Search, TOC, and an asset-bearing Offline v3 package. The combined merge and Offline files passed 15/15; this is automated API/package evidence, not authenticated browser or production verification.
- Attachment UUID rewriting during Conversation Merge now keeps every target projection coherent: MessageVersion display/plain/hash and block JSON, RenderBlock plain/data/sanitized HTML, and the target Message current hash all use the rewritten IDs. Current Message hashes are updated in bounded batches rather than retaining the source hash. Focused merge coverage passed 9/9 and the full API suite passed 426 with 6 skips. This is implemented locally, not committed, deployed, or production-verified.
- Conversation and Message Merge now share source-version attachment integrity validation before any merge output is published. Missing/inactive/cross-conversation Attachments, missing occurrences, missing RenderBlocks, and occurrence/block mismatches fail closed; synchronous Message Merge returns `422 attachment_integrity_invalid`, while a failed background Conversation Merge rolls back its processing target and preserves both sources. Focused merge coverage passed 17/17 and the full API suite passed 426 with 6 skips. This is implemented locally, not committed, deployed, or production-verified.
- Historical merged-attachment UUID repair is now available as an explicit dry-run/apply operator flow. It considers only unique `source_attachment_id` provenance inside the selected Conversation, requires a fresh confirmation token after row locking, refuses all unresolved or ambiguous mappings, preserves history through a new MessageVersion, and rebuilds Search/TOC. It does not guess by filename, order, or similar content and has not been run against user data. Focused repair coverage passed 2/2, the full API suite passed 418 with 6 skips, and Alembic remains at the single `20260829_0029` head.
- Attachment integrity auditing is now complete across current and historical MessageVersions. The rollback-only `python -m scripts.attachment_integrity_report` emits aggregate global counts by default and identity details only for an explicitly selected Conversation; it never returns body text or guesses repairs. Missing Attachment/current occurrence/block and stale-version links are covered by 8 focused attachment/merge tests.
- The current retention boundaries are consolidated in [Task And Offline Retention Contract](docs/system/RETENTION_CONTRACT.md): terminal Task Center results default to 600 seconds, canonical server Offline packages have no time TTL, artifact cleanup has a 24-hour technical grace with explicit manual apply, and browser Offline Library data is browser-managed. Export/draft/upload 24-hour values remain separate.
- Protected task diagnostics now retain the existing bounded 500-completion sample and add separate fixed-bucket distributions for queue wait and execution duration (1/5/15/60/300 seconds), alongside the backward-compatible averages. This adds no query, identifier, task history, or migration; focused diagnostics coverage passed 8/8.
- Adaptive Import now has content-safe regressions derived from the supplied unknown-format examples: ordinary/reference documents resolve to `NOT_MAPPABLE` with `OPEN_RESCUE` and no mapping draft, while zero-byte Markdown is rejected before session creation. A read-only check of the actual example directory also confirmed Native v2 remains `SUPPORTED` and stable message structures remain `MAPPABLE`; the Adaptive Import API file passed 32/32.
- Reader locate feedback is now regression-protected as a bounded first-visible-line pulse, with a 3px message-level fallback and a static reduced-motion variant. It clears after 720ms or immediately on user scroll/pointer input; the focused Reader contract gate passed 8/8, while authenticated production interaction remains `NOT VERIFIED`.
- Reader performance evidence now separates initial first-content paint, locator resolution, and target mount/alignment. The browser contract exposes only bounded duration, outcome, and local/remote path enums through Performance entries and a CustomEvent; it contains no object IDs or user content. Web lint, typecheck, production build, and the focused 7/7 Reader contract browser gate passed; authenticated production timing collection remains `NOT VERIFIED`.
- Offline package jobs now expose truthful store-level phases through the existing task monitor: conversation messages, section index, offline search index, annotations, reading state/notes, attachment index, conversation packaging, attachment writes, package validation, and publishing. The phase callback preserves existing progress fields and task result semantics; API offline coverage passed 6/6 and the full Task Center browser file passed 9/9 in production-equivalent Chromium. No new task history or persistence was added.
- Offline package import now preflights a bounded storage-footprint estimate rather than comparing quota only with compressed or expanded ZIP bytes. The estimate includes IndexedDB structured-clone/index overhead, Cache Storage asset overhead, per-record index cost, and a fixed reserve; insufficient space returns `QUOTA` before any IndexedDB write and preserves the last readable package. Web lint, typecheck, production build, a focused Chromium preflight assertion, and the complete 12/12 PWA negative matrix passed; production browser acceptance remains `NOT VERIFIED`.
- Offline Library download failures now retain the failed scope in local UI state and expose an explicit Retry action. The existing atomic package importer still preserves the last readable copy; this adds no journal or partial-package semantics. Typecheck and lint passed. The Library browser suite timed out during local PWA startup, so this retry interaction is `NOT VERIFIED` in browser.
- Import close/return remains covered by the existing portal dialog and shared focus contract: `useDialogFocus` restores the original opener after cancel/close, while active Adaptive sessions use sessionStorage for re-entry. No new route or persistent state was added; authenticated owner browser verification remains `NOT VERIFIED`.
- Offline package schema diagnostics now distinguish legacy v1 compatibility from v2+ store shape errors before opening a write transaction. Required derived stores and optional object fields receive bounded `MALFORMED` messages; the v1 reader path remains permissive. A dedicated negative-PWA Chromium check passed 1/1, with no migration or new error code.
- The global Task Center now uses the same task monitor and cancellation contract on desktop and mobile. Below 640px it is a safe-area-aware 100dvh focused surface; desktop retains the bounded centered dialog. Production-equivalent Chromium at 375x812 passed global-sidebar entry, full-height bounds, Escape dismissal without cancellation, and reopening the same active task. The focused Task Center/batch interaction file passed 7/7; authenticated production acceptance remains `NOT VERIFIED`.
- Terminal task results now have a bounded, configurable 600-second visibility window (`TASK_TERMINAL_RESULT_RETENTION_SECONDS`) in `/api/tasks/active`. Recent committed, failed, and cancelled results remain available after route changes or refresh, while diagnostics expose only redacted aggregate counts and ages; no permanent task history or migration was added. Focused API coverage passed 9 tests, the full API suite passed 412 tests with 6 skips, and the focused Task Center browser file passed 8/8 in production-equivalent Chromium.
...
### 2026-09-01 locator failure-state refinement

- Source SHA at review: `1f7117fe509fccb09331b669f2b719ae82f8cf49` plus current dirty worktree changes.
- LOC-005 implementation: Reader navigation failures now retain the current body and render reason-specific recovery copy for attachment-invalid, ambiguous, deleted/merged, stale-version, and generic load failures. Retry and message-level fallback actions remain available.
- Existing locator v2 anchor compatibility (`locator_version: 2`, message/version/block identity and canonical offsets) was preserved. Full server-side resolver unification remains LOC-001/LOC-002 scope.
- Verification: `corepack pnpm run typecheck` PASS; `corepack pnpm run lint` PASS. Authenticated browser execution NOT VERIFIED because the local API/worker fixture is unavailable.

### 2026-09-01 CI workflow revalidation

- CI-011 closed after confirming the standard Web build and PWA-negative build intentionally use separate output directories and environment flags; sharing `.next` would risk testing the wrong artifact. Both builds remain independently timed and uploaded as evidence.
- CI-018 closed as already current: release/performance workflows use maintained checkout/setup/upload action majors and no runtime-warning migration is outstanding at this source SHA.
- Backlog counts: 118 completed, 16 remaining, 0 blocked.

### 2026-09-01 release artifact boundary check

- CI-019 implemented in `.github/workflows/build-release-images.yml`: a separate `inspect-release-artifact` job downloads the exact SHA/run-attempt artifact, validates required manifest/archive files, reloads API/worker/migration/Web images and checks their OCI revision labels before release handoff.
- The job is verification-only and does not deploy, mutate registries, or change runtime topology. YAML parsing passed locally; GitHub-hosted execution remains pending the next workflow run.
- Backlog counts: 119 completed, 15 remaining, 0 blocked.

### 2026-09-01 offline mobile verification note

- A 375x812 Library/Reader/TOC/files acceptance case was added as a fixture for OFF-010 but is currently skipped because the local PWA server did not reach its offline-shell-ready state in isolation. Existing 360/390px offline coverage remains unchanged and passing where its environment is available; OFF-010 stays `Candidate` / `NOT_VERIFIED` rather than being promoted from an unavailable run.

### 2026-09-01 offline packaging timing evidence

- OFF-009 implemented: BackgroundJob offline package results now include bounded `phase_durations_ms` for existing packaging phases, allowing store/phase bottlenecks to be measured without persisting content, identifiers or filesystem paths. No worker, transaction or schema behavior changed.
- Focused API coverage (`tests/test_offline_annotations_api.py -k "catalog_and_package_are_downloadable or store_level_progress"`) passed 2/2. Production timing collection is not yet available.
- Backlog counts: 120 completed, 14 remaining, 0 blocked.

### 2026-09-01 locator media-settle optimization

- Exact mounted Reader targets now use a 900ms media-settle guard before alignment; network/remapped targets retain an 1800ms guard. This removes the previous five-second image decode wait from ordinary citation/attachment jumps while preserving a bounded layout-stability check.
- Web lint, typecheck and Reader location tests remain green after the change. No locator identity, offset or fallback semantics changed.

### 2026-09-01 server locator foundation (local)

- Added the owner-scoped `POST /api/conversations/{conversation_id}/resolve-locator` contract and canonical resolver for message, current-version, render-block, attachment occurrence, quote/context and offset validation. Historical version requests are explicitly remapped to the current Reader version; missing or ambiguous attachment targets fail closed.
- Remote Reader target-context loading now carries the resolver DTO and uses its effective block and Unicode offset when mounting and highlighting. Attachment occurrence targets bypass the local block-index shortcut so Files/Viewer jumps cannot silently select a sibling occurrence.
- Share Reader now exposes the same resolver contract through a token-scoped endpoint; the route validates the share's message scope before resolving and keeps the existing public fallback for older deployments. Offline Reader remains local-only.
- Added two API resolver regressions and retained the existing Reader location-failure coverage. API full suite (`445 passed, 6 skipped`), Web lint/typecheck/build and focused Reader/attachment browser checks pass. Share now uses a token-scoped resolver and Offline uses an equivalent local resolver; DOM Range remains presentation-only and message-only dialogue navigation remains an intentional compatibility fallback. LOC-001/LOC-002 are completed locally; authenticated production verification remains separate.

### 2026-09-01 pnpm cache evidence

- CI-016 implemented locally: release and performance workflows now use an explicit `actions/cache@v4` pnpm-store step keyed by `pnpm-lock.yaml` and upload a redacted `pnpm-cache.txt` hit/miss record with the existing evidence artifacts. No dependency or deployable output changed; workflow YAML parses successfully.

### 2026-09-01 bounded deployment state and transfer cleanup (local)

- DEP-004 completed locally: deployment pointers are documented as operator-owned `/etc/chat-reader/release-state/current-images.env` and `rollback-images.env`; `deploy/verify_release_state.sh` is read-only and requires distinct safe `RELEASE_SHA` values.
- DEP-006 completed locally: `deploy/cleanup_release_transfer.py` reports direct-child release transfer candidates by default and only removes them with explicit `--execute`, preserving exact `--keep` current/rollback artifact names and refusing symlink targets.
- No production host state was changed and no deployment was performed.
