...
## 2026-08-12 Rich Markdown Scientific-inline Closure Candidate
...
- A second reported conversation contains standalone ChatGPT bracket formulas using common scientific commands such as `lambda`, `mathbb`, `in`, `langle`, `neq`, `xrightarrow`, `mathrm` and `times`. The v2 allowlist omitted them. `ai-rich-markdown-v3` extends only the bounded standalone-bracket compatibility grammar; code, HTML, prose, currency and canonical source remain unchanged.
- Production-equivalent verification: parser/shared contract `13/13`; Reader/Editor/security/stress `5/5`; exact read-only source copies for both reported shapes `1/1` each; Web lint/typecheck/build PASS; full API `235 passed / 4 fixture-gated skipped`; default PWA `57 passed / 36 environment-gated skipped`; Alembic single head `20260806_0021`. Both copies rendered MathML with zero local math errors or page-level overflow and were deleted through the product API. Production deployment and read-only regression remain pending for this candidate.
