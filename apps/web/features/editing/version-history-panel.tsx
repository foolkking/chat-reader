...
import { deleteMessageVersion, getMessageVersions, getTask, selectMessageVersion } from "../../lib/api";
import type { MessageListItem, MessageVersionHistoryResponse } from "../../lib/types";
...
export function VersionHistoryPanel({ messageId, currentVersionId, onChanged }: {
  ...
  const renderedMessageIdRef = useRef(messageId);
  ...
  const derivedTaskQuery = useQuery({
    queryKey: ["version-derived-task", derivedJobId],
    queryFn: () => getTask(derivedJobId!),
    enabled: Boolean(derivedJobId),
    refetchInterval: (query) => {
      const status = query.state.data?.status;
      return !status || ["queued", "processing", "cancelling"].includes(status) ? 1500 : false;
    },
  });
  ...
  useEffect(() => {
    const task = derivedTaskQuery.data;
    if (!task || !derivedJobId || !["committed", "failed", "cancelled"].includes(task.status)) return;
  ...
  }, [derivedJobId, derivedTaskQuery.data, queryClient]);

  useEffect(() => {
    if (derivedTaskQuery.isError) setDerivedJobId(null);
  }, [derivedTaskQuery.isError]);

  function updateCurrentVersion(nextVersionId: string) {
    queryClient.setQueryData<MessageVersionHistoryResponse>(["message-versions", messageId], (existing) => {
      if (!existing) return existing;
      return {
        ...existing,
        current_version_id: nextVersionId,
        items: existing.items.map((item) => ({ ...item, is_current: item.id === nextVersionId })),
      };
    });
  }
  ...
  async function select(index: number) {
    ...
    if (!target || target.id === current?.id || operationInFlightRef.current) return;
    operationInFlightRef.current = true;
    setBusy(true);
    try {
      const response = await selectMessageVersion(messageId, target.id);
      if (renderedMessageIdRef.current !== messageId) return;
      updateCurrentVersion(response.current_version_id);
      setDerivedJobId(response.derived_status === "queued" ? (response.derived_job_id ?? null) : null);
  ...
  async function remove() {
    if (!current?.can_delete || operationInFlightRef.current) return;
      ...
      queryClient.setQueryData<MessageVersionHistoryResponse>(["message-versions", messageId], (existing) => {
        ...
        return {
          ...
          items: existing.items
            .filter((item) => item.id !== response.deleted_version_id)
            .map((item) => ({ ...item, is_current: item.id === response.current_version_id })),
        };
      });
      setDerivedJobId(response.derived_status === "queued" ? (response.derived_job_id ?? null) : null);
      void queryClient.invalidateQueries({ queryKey: ["active-tasks"] });
  ...
  return (
    <div className="relative flex items-center gap-1" data-testid="message-version-control">
      ...
      {open && current ? <section className="absolute right-0 top-11 z-50 w-[min(26rem,calc(100vw-2rem))] rounded-lg border border-ui bg-raised p-3 shadow-2xl" aria-label={zh ? "版本详情面板" : "Version details panel"}>
        ...
        {derivedJobId ? <p className="mt-2 text-xs text-secondary" role="status">{zh ? "正文已更新，搜索和章节目录正在后台更新。" : "Content updated. Search and sections are updating in the background."}</p> : null}
