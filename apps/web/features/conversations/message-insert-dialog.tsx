...
export function MessageInsertDialog({
  ...
  revision?: number;
  onClose: () => void;
  onSubmitted: (messages: MessageListItem[]) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-[270] flex items-end justify-center bg-[var(--overlay)] sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="message-insert-title">
      ...
      <form onSubmit={submit} className="relative flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl border border-ui bg-raised shadow-2xl sm:rounded-2xl">
        ...
        <div className="min-h-0 space-y-4 overflow-y-auto p-4">
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-medium text-primary">位置<select value={position} onChange={(event) => setPosition(event.target.value as "before" | "after")} className="mt-1.5 h-11 w-full rounded-lg border border-ui bg-page px-3 text-sm"><option value="before">插入到此消息之前</option><option value="after">插入到此消息之后</option></select></label><label className="text-sm font-medium text-primary">方式<select value={mode} onChange={(event) => setMode(event.target.value as "single" | "pair")} className="mt-1.5 h-11 w-full rounded-lg border border-ui bg-page px-3 text-sm"><option value="single">插入一条消息</option><option value="pair">插入 User + Assistant</option></select></label></div>
