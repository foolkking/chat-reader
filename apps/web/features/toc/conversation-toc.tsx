...
export function ConversationToc({ conversationId, activeMessageId, activeItems = [], observerKey, activeBlockId, items, mode = "panel", onNavigate }: { conversationId: string; activeMessageId?: string | null; activeItems?: TocItem[]; observerKey?: string; activeBlockId?: string | null; items?: TocItem[]; mode?: "panel" | "sheet"; onNavigate?: (item: TocItem) => void | Promise<void>; }) {
  ...
  return <aside aria-label={t("sectionToc")} className="w-full border-l border-ui pl-[clamp(0.75rem,1cqi,1.25rem)]"><div className="mb-3 flex items-center justify-between"><h2 className="text-[15px] font-semibold text-secondary">{t("sectionToc")}</h2><span className="text-[13px] text-secondary">{visibleItems.length}</span></div><div className="reader-aux-scroll max-h-[calc(100dvh-8rem)] overflow-y-auto pr-1 text-[14px] leading-6">{body}</div></aside>;
}
