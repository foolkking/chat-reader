...
export function TaskCenterDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[315] flex items-end justify-center bg-[var(--overlay)] p-0 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="task-center-title" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={rootRef} tabIndex={-1} onPointerDown={(event) => event.stopPropagation()} className="relative flex max-h-[min(88dvh,52rem)] w-full flex-col overflow-hidden rounded-t-2xl border border-ui bg-raised shadow-2xl outline-none sm:max-w-2xl sm:rounded-xl">
