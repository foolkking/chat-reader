...
export default function ConversationLoading() {
  ...
  return (
    <div className={`flex min-h-0 min-w-0 overflow-hidden bg-[#f7f7f8] ${workspace.embedded ? "h-full w-full flex-1" : "h-screen w-screen"}`}>
      {!workspace.embedded ? <ProjectSidebar mobileOpenSignal={mobileSidebarOpenSignal} showMobileTrigger={false} readerMode /> : null}
      <section className="flex min-w-0 flex-1 flex-col">
        <MobilePageHeader
          title="Chat Reader"
          description={zh ? "正在恢复阅读位置…" : "Restoring your reading position…"}
          onOpenSidebar={() => workspace.embedded ? undefined : setMobileSidebarOpenSignal((value) => value + 1)}
        />
