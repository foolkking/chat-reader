...
export function ImportFormatSettings() {
  ...
  return (
    <section className="border-t border-ui pt-3">
      <button type="button" onClick={() => setOpen((value) => !value)} className="flex min-h-9 w-full items-center justify-between text-left text-sm font-medium text-primary" aria-expanded={open}>
      ...
      {open ? <div className="mt-3 space-y-3">{query.isLoading ? <p className="text-xs text-secondary">正在读取导入格式…</p> : null}{query.isError ? <p role="alert" className="text-xs text-[var(--danger)]">{query.error.message}</p> : null}{query.data?.map((profile) => <FormatRow key={profile.id ?? profile.key} profile={profile} />)}</div> : null}
    </section>
  );
}

function FormatRow({ profile }: { profile: ImportFormatProfile }) {
  ...
  return (
    <article className="border-l-2 border-ui pl-3">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs">
        ...
        {profile.kind === "LEARNED" && profile.id ? <button type="button" onClick={() => openImportDialog({ repairProfileId: profile.id! })} className="inline-flex items-center gap-1 font-medium text-accent underline"><Wrench className="h-3.5 w-3.5" />修复格式</button> : null}
