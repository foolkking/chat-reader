  ...
  return (
    <main className="flex h-screen w-screen overflow-hidden bg-page text-primary">
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden"><div className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-[clamp(1rem,2vw,2rem)] py-8">{!isArchivedMode ? <div className="md:hidden"><RecentItems compact /></div> : null}{isArchivedMode ? <ArchivedProjectList /> : null}<ConversationList mode={mode} onImportClick={openImportDialog} /></div></div>
      </section>
    </main>
  );
}
