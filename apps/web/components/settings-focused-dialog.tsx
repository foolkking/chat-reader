...
export function SettingsFocusedDialog({ category, onClose, restoreFocus }: { category: SettingsCategory; onClose: () => void; restoreFocus: () => HTMLElement | null }) {
  ...
  const title = category === "data" ? t("dataArchive") : category === "security" ? t("accountSecurity") : category === "cleanup" ? t("noiseRuleLibrary") : category === "skills" ? t("skillManagement") : category === "access" ? (t("settings") === "\u8bbe\u7f6e" ? "\u7528\u6237\u4e0e\u8bbf\u95ee" : "Users & access") : t("importFormats");
  ...
  return createPortal(
    <div className="fixed inset-0 z-[320] flex items-end justify-center bg-[var(--overlay)] p-0 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="settings-focused-title" onPointerDown={(event) => { if (event.target === event.currentTarget) void requestClose(); }}>
      <div ref={rootRef} tabIndex={-1} className={`relative flex max-h-[min(92dvh,56rem)] w-full flex-col overflow-hidden rounded-t-2xl border border-ui bg-raised shadow-2xl outline-none sm:rounded-xl ${category === "access" ? "sm:max-w-3xl" : "sm:max-w-2xl"}`}>
