...
export function SkillSettings({ focused = false, onDirtyChange }: { focused?: boolean; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const clone = useMutation({ mutationFn: async (skill: SkillRead) => {
    if (!skill.content_url) throw new Error("Skill content is unavailable");
    const response = await fetch(skill.content_url, { credentials: "same-origin" });
    if (!response.ok) throw new Error(`Skill content returned ${response.status}`);
    const content = await response.text();
    return createSkill({ category: skill.category, locale: skill.locale, name: `${skill.name} - Copy`, file: new File([content], `${skill.name}.md`, { type: "text/markdown;charset=utf-8" }) });
  }, onSuccess: () => { setNotice("Skill 已克隆到我的 Skill；不会自动设为首选。"); void queryClient.invalidateQueries({ queryKey: ["skills"] }); } });
  ...
  return <section className={focused ? "space-y-4" : "space-y-3"}>
    ...
    <div className="space-y-2">{query.isLoading ? <p className="text-xs text-secondary">正在读取 Skill…</p> : null}{rows.map((skill) => <SkillRow key={skill.id} skill={skill} onView={() => setViewing(skill)} onClone={() => clone.mutate(skill)} onSelect={() => select.mutate(skill.source === "USER" ? skill.id : null)} onToggle={() => skill.source === "USER" && toggle.mutate({ id: skill.id, status: skill.status === "ACTIVE" ? "DISABLED" : "ACTIVE" })} onDelete={() => skill.source === "USER" && window.confirm(`删除“${skill.name}”？`) && remove.mutate(skill.id)} />)}</div>
    ...
    {rows.some((skill) => skill.source !== "USER" && skill.content_url) ? <div className="flex flex-wrap gap-2">{rows.filter((skill) => skill.source !== "USER" && skill.content_url).map((skill) => <button key={`clone-${skill.id}`} type="button" onClick={() => clone.mutate(skill)} className="btn-secondary min-h-9 px-3 text-xs text-accent">克隆“{skill.name}”到我的 Skill</button>)}</div> : null}
    {viewing ? <SkillContentDialog skill={viewing} onClose={() => setViewing(null)} /> : null}
  </section>;
}

function SkillRow({ skill, onView, onClone: _onClone, onSelect, onToggle, onDelete }: { skill: SkillRead; onView: () => void; onClone: () => void; onSelect: () => void; onToggle: () => void; onDelete: () => void }) {
